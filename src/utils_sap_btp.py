"""
Utility functions for the Crawl4AI MCP server with SAP BTP AICore integration.

This module replaces OpenAI dependencies with SAP BTP AICore services.
"""
import os
...
# Import SAP BTP AICore client instead of OpenAI
...
def create_embeddings_batch(texts: List[str]) -> List[List[float]]:
    """
    Create embeddings for multiple texts using SAP BTP AICore.
    ...
    for retry in range(max_retries):
        try:
            aicore_client = get_aicore_client()
            response = aicore_client.create_embeddings(texts)
            ...
            if retry < max_retries - 1:
                ...
                for i, text in enumerate(texts):
                    try:
                        individual_response = aicore_client.create_embeddings([text])
...
def create_embedding(text: str) -> List[float]:
    """
    Create an embedding for a single text using SAP BTP AICore.
...
def generate_contextual_embedding(full_document: str, chunk: str) -> Tuple[str, bool]:
    ...
    """
    model_choice = os.getenv("SAP_BTP_AICORE_CHAT_MODEL", "gpt-4o")
...
Please give a short succinct context to situate this chunk within the overall document for the purposes of improving search retrieval of the chunk. Answer only with the succinct context and nothing else."""
        ...
        # Call the SAP BTP AICore API to generate contextual information
        aicore_client = get_aicore_client()
        messages = [
        ...
        response = aicore_client.create_chat_completion(
            messages=messages,
            model=model_choice,
            temperature=0.3,
            max_tokens=200
...
def extract_source_summary(source_id: str, content: str, max_length: int = 500) -> str:
    """
    Extract a summary for a source from its content using SAP BTP AICore.
    
    This function uses the SAP BTP AICore API to generate a concise summary of the source content.
...
"""
    ...
    try:
        # Call the SAP BTP AICore API to generate the summary
        aicore_client = get_aicore_client()
        messages = [
        ...
        print(f"Error generating summary with SAP BTP AICore for {source_id}: {e}. Using default summary.")
        return default_summary
